...
echo "   Verify: kubectl cluster-info"
...
echo "   Verify: kubectl get pods -n argocd -w"
echo "   (wait until all pods are Running/Completed)"
echo ""
echo "   After startup - get admin password:"
echo "   kubectl -n argocd get secret argocd-initial-admin-secret -o jsonpath='{.data.password}' | base64 -d && echo"
echo ""
echo "   Port forward for UI:"
echo "   kubectl port-forward -n argocd svc/argocd-server 8080:443"

# Step 3: Default StorageClass gp3
echo ""
...
echo "   (look for '(default)' next to gp3)"
